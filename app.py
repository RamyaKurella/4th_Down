# app.py  – 4-Down Team-Ratings Explorer
...
with c2:
    st.markdown("## 🧐 NCAA 4th-Down Success Explorer 🧐")

st.markdown("""
<details open>
<summary style='font-size:18px; color:#58a6ff; cursor:pointer;'>
<strong>🧐 &nbsp;What does this application do?</strong>
</summary>

This application evaluates **4th-down conversion performance** for NCAA football teams with two machine-learning models:

* **Baseline model** – predicts success using only on-field context (distance-to-go, yard-line, period, score, etc.).  
* **Enhanced model** – adds team-specific offensive and defensive strength coefficients, estimated via Ridge regression on drive-level data.

By contrasting actual results with these expectations, you can quickly spot programs that thrive—or struggle—when the game is on the line.
...
with tab_data:
    ...
    years = st.multiselect("Years",  range(2000, 2025), default=st.session_state.get("years", [2022]))
    weeks = st.multiselect("Weeks",  range(1, 16), default=st.session_state.get("weeks", [1]))
    ...
    if st.button("🚀 Fetch Data"):
        ...
        with st.spinner("Downloading drives & plays …"):
            st.session_state.drives = pd.concat([
                u.fetch_drives(y, w) for y in years for w in weeks], ignore_index=True)
            st.session_state.plays = pd.concat([
                u.fetch_plays (y, w) for y in years for w in weeks], ignore_index=True)

        st.session_state.update(dict(
            years=years, weeks=weeks,
            coeffs=pd.DataFrame(), ratings_no=pd.DataFrame(), ratings_with=pd.DataFrame()
        ))

        st.success(f"Fetched **{len(st.session_state.drives):,} drives** and **{len(st.session_state.plays):,} plays** ✔️")
    ...
    if not st.session_state.drives.empty:
        with st.expander(f"📄 Drives sample ({len(st.session_state.drives):,} rows)"):
            ...
            st.download_button("⬇ Download ALL drives (.csv)",
                               st.session_state.drives.to_csv(index=False), "drives_raw.csv", "text/csv")
    ...
    if not st.session_state.plays.empty:
        with st.expander(f"📄 Plays sample ({len(st.session_state.plays):,} rows)"):
            ...
            st.download_button("⬇ Download ALL plays (.csv)",
                               st.session_state.plays.to_csv(index=False), "plays_raw.csv", "text/csv")
...
with tab_no:
    ...
    if st.session_state.plays.empty:
        ...
        if st.button("⚙️ Run Ratings (Without Coeffs)"):
            ...
            st.session_state.ratings_no = (
                preds.groupby("offense")
                     ...
                     .reset_index()
                     .assign(Actual=lambda d:d.Actual*100, Pred=lambda d:d.Pred*100)
        ...
        if not st.session_state.ratings_no.empty:
            ...
            st.download_button("⬇ CSV", df.to_csv(index=False), "team_ratings_without_coeffs.csv","text/csv")
...
with tab_coeff:
    ...
    if st.session_state.drives.empty:
        ...
        if not st.session_state.coeffs.empty:
            ...
            st.download_button("⬇ CSV", st.session_state.coeffs.to_csv(index=False),
...
with tab_with:
    ...
    if st.session_state.coeffs.empty or st.session_state.plays.empty:
        ...
        if not st.session_state.ratings_with.empty:
            ...
            st.dataframe(df.round(2), use_container_width=True)
            st.download_button("⬇ CSV", df.to_csv(index=False), "team_ratings_with_coeffs.csv","text/csv")
